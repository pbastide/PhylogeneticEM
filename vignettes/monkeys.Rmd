...
To keep the computations down, we keep here this default, but this number should be raised on a real application.
...
Here, the solution selected has four shifts. 
As shown above, the criterion is however a bit flat around its minimum.
...
Indeed, when applied on the rotated dataset, `PhyloEM` selects for a solution with 7 shifts, instead of 4.
...
plot(res, params = params_process(res, K = 4), no.margin = TRUE)
plot(res_rot, params = params_process(res, K = 4), no.margin = TRUE)
```
...
This function takes the first fit has a reference (here, with the original data), and, when the second fit has a better likelihood, it takes its solution, and rotate it back in the original space.
...
We can see here that the solution with 4 shifts is now selected again.
...
the application of several pseudo-orthonormalisation techniques on the original dataset were needed to
...
One should hence be careful when using and interpreting such a model, and the use of this function is not encouraged for a first analysis.
...
res_neg <- PhyloEM(phylo = monkeys$phy,
                   ...
                   parallel_alpha = TRUE,
                   Ncores = 2,
                   nbr_alpha = 20,           ## 10 negative, 10 positive
                   allow_negative = TRUE)    ## allow negative alpha in the grid
```

We can extract the parameter of the fit with no shifts, that is equivalent to an EB model:
```{r}
params_process(res_neg, K = 0)
```
The "selection strenght" $\alpha$ is linked with the rate $r$ of the EB by the relation: $r = 2\alpha$.
...
Here, the same solution with 5 shifts (and a positive alpha) is selected.
