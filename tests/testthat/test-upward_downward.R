...
test_that("Upward Downward - BM - with pheno error", {
  testthat::skip_on_cran()
  set.seed(17920902)
  ntaxa = 100
  tree <- TreeSim::sim.bd.taxa.age(n = ntaxa, numbsim = 1, lambda = 0.1, mu = 0, 
  ...
  paramsSimu <- list(variance = variance,
                     shifts = shifts,
                     root.state = root.state,
                     pheno_error = diag(0.2, p, p))
  attr(paramsSimu, "p_dim") <- p
  attr(paramsSimu, "ntaxa") <- ntaxa
  ...
  tree_errors <- add_zero_length_tips(tree)
...
test_that("Upward Downward - estimateEM - BM - pheno_error", {
  testthat::skip_on_cran()
  set.seed(17920902)
  ntaxa = 100
  tree <- TreeSim::sim.bd.taxa.age(n = ntaxa, numbsim = 1, lambda = 0.1, mu = 0, 
  ...
  expect_warning(res_old_err <- estimateEM(phylo = tree, 
                                           Y_data= traits, 
                                           process = "BM", 
                                           pheno_error = diag(0.2, p, p),
                                           method.variance = "simple", 
  ...
  expect_warning(res_new_err <- estimateEM(phylo = tree, 
                                           Y_data= traits, 
                                           process = "BM",
                                           pheno_error = diag(0.2, p, p),
                                           method.variance = "upward_downward", 
  ...
  expect_that(res_new_err, equals(as.vector(res_old_err)))
})
...
test_that("Upward Downward - PhyloEM - BM - error", {
  testthat::skip_on_cran()
  set.seed(17920902)
  ntaxa = 50
  tree <- TreeSim::sim.bd.taxa.age(n = ntaxa, numbsim = 1, lambda = 0.1, mu = 0, 
...
test_that("Upward Downward - scOU - fixed root - pheno_error", {
  testthat::skip_on_cran()
  set.seed(17920902)
  ntaxa = 100
  tree <- TreeSim::sim.bd.taxa.age(n = ntaxa, numbsim = 1, lambda = 0.1, mu = 0,
  ...
  root.state <- list(random = FALSE,
                     ...
                     exp.root = NA,
                     var.root = NA)
  shifts <- list(edges = c(12, 173, 165),
                 values = cbind(c(4, -10, 3, 1),
                                c(-5, 5, 0, 1),
                                c(4, -10, 3, -1)),
                 relativeTimes = 0)
